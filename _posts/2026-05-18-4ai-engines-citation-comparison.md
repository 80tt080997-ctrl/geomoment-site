...
summary: |
  - 네 엔진은 모두 _출처 표시_를 말하지만, **구현 철학이 달라요** — ChatGPT는 inline citations + Sources 패널, Perplexity는 _every answer = direct links_, Gemini는 groundingMetadata 메타데이터 구조, Claude는 cited_text 토큰 별도 회계.
  ...
  - 한국 시장은 **ChatGPT 2,293만 명**이 압도적 1위 (와이즈앱 2026-02). 다만 _4사 모두 측정_해야 답변 엔진 가시성 절반 이상 확보.
  - Peec AI 3천만 소스 분석 — AI 검색 전체 상위 도메인은 **Reddit · YouTube · LinkedIn · Wikipedia · Forbes · G2 · Yelp · Facebook · Medium · TechRadar**.
  - 학술 결론(ACL NAACL 2025): _sentence-level citation_이 90% accuracy로 가장 신뢰 가능한 단위 — 청크 단위 처방과 정확히 같은 방향.
---

지난 글에서 GeoMoment 블로그는 _AI 검색 시대의 SEO 변화_를 다뤘어요 ([4편 — AI Overview · AI Mode 등장 후 SEO](/blog/ai-overview-and-seo/)). 한 가지 짚지 않고 넘어간 게 있어요 — _AI 답변 엔진은 한 곳이 아니라 네 곳_이라는 사실입니다.

같은 질문 _"서울 강북에서 50인 회의실 어디가 좋아?"_ 를 ChatGPT · Perplexity · Gemini · Claude 네 엔진에 던지면 _완전히 다른 출처_를 인용해요. ChatGPT는 위키·블로그 매체 위주, Perplexity는 Reddit·G2 같은 커뮤니티·리뷰형, Gemini는 브랜드 사이트, Claude는 권위 매체. 같은 정보를 찾는데도 _어떤 페이지가 답변에 들어가느냐_가 엔진마다 다른 거예요.

이 차이가 마케터에게 중요한 이유는 단순해요 — **우리 브랜드는 _어떤 엔진_에서 _어떤 종류의 페이지_로 인용되는지** 알아야 _어디부터 측정·최적화할지_ 결정할 수 있거든요. 한국 시장은 ChatGPT 사용자가 2,293만 명으로 압도적 1위인데, 그럼에도 _4사 모두_ 측정해야 하는 이유도 이 글에서 짚어볼게요.
...
**ChatGPT는 _질문을 받으면 필요할 때 웹을 검색해서 답변에 인라인 인용(inline citations)과 Sources 패널을 함께 붙이는_ 방식이에요.** OpenAI가 2024년 10월에 ChatGPT Search를 정식 발표한 이후의 표준 동작입니다.

OpenAI는 [_Introducing ChatGPT search_ (2024-10-31)](https://openai.com/index/introducing-chatgpt-search/)에서 이렇게 짚어요. *"ChatGPT can now search the web in a much better way than before. You can get fast, timely answers with links to relevant web sources, which you would have previously needed to go to a search engine for."* — 검색 엔진을 따로 가는 대신 ChatGPT 안에서 _답변 + 출처 링크_를 한 번에 받는다는 뜻이에요. 이 발표에서 Associated Press · Reuters · The Atlantic · Financial Times · Time 등 **14개 출판사 제휴**도 공개됐습니다.

출처 노출 UI는 OpenAI Help Center가 더 구체적이에요. [_ChatGPT search for Enterprise and Edu_ 문서](https://help.openai.com/en/articles/10093903-chatgpt-search-for-enterprise-and-edu)는 *"ChatGPT responses that use search **will contain** inline citations"* 라고 명시합니다. _will contain_ — 검색을 쓴 답변에는 인라인 인용이 _포함된다_는 단정적 표현이에요. UI는 세 층으로 작동해요. 첫째, 답변 본문 안의 _인라인 인용 마크_. 둘째, 인용 위에 _마우스를 호버하면 보이는 출처 정보_. 셋째, 답변 끝의 _Sources 버튼을 누르면 펼쳐지는 출처 패널_.

2025년 10월에는 [ChatGPT Atlas](https://help.openai.com/en/articles/12591856-chatgpt-atlas-release-notes)도 출시됐어요. *"ChatGPT Atlas is a new web browser with ChatGPT built in."* — ChatGPT가 브라우저 안에 내장된 형태로, 사이드바에 _Ask ChatGPT_ · Agent Mode · 브라우저 메모리 기능을 갖습니다. 답변의 인용 UI 자체는 ChatGPT Search 문서의 패턴을 따라요.

여기서 마케터 시점으로 옮기면 한 가지 핵심이 나옵니다 — **ChatGPT는 _출처 링크를 답변에 표시하는 것을 표준 동작으로 설계_했어요.** 즉 _우리 페이지가 답변 안에 인용되면 사용자가 클릭해서 들어올 수 있는 경로가 자동으로 생긴다_는 뜻입니다. 4편 글에서 짚은 Seer 데이터(_AIO 인용 시 organic 클릭 +35%_)와 정확히 같은 메커니즘이에요.

ChatGPT가 어떤 출처를 우선 인용하는지는 H2-5 비교 단락에서 자세히 짚어볼게요. 짧게 미리 짚으면, **ChatGPT의 최다 인용 도메인은 Wikipedia**(Profound 측정에서 _전체 인용의 7.8%_, _상위 10개 출처 중 47.9%_) — 백과·기관형 출처에 대한 신뢰가 가장 큰 엔진입니다.

## Perplexity는 어떻게 답변하고 어떻게 인용하나요?

**Perplexity는 _모든 답변에 출처 링크를 기본 전제로 붙이는_ 답변 엔진(answer engine)이에요.** ChatGPT가 _필요할 때 검색을 켜는_ 어시스턴트에 가깝다면, Perplexity는 _검색이 곧 답변 생성의 일부_인 구조로 처음부터 설계된 게 차이예요.

[Perplexity 공식 정의](https://www.perplexity.ai/help-center/en/articles/10354917-what-is-an-answer-engine-and-how-does-perplexity-work-as-one)는 한 문장으로 정리돼요. *"Perplexity serves as an answer engine by searching the web, identifying trusted sources, and synthesizing information into clear, up-to-date responses. Unlike traditional search engines, which make you sift through a list of links, Perplexity delivers the insights you're looking for in one place."* — 전통 검색이 _링크 목록을 보여주는 것_과 달리, Perplexity는 _신뢰할 출처를 식별해서 정보를 종합한 답변_을 한곳에 내놓는다는 입장이에요.

핵심 동작 원칙은 [_What is Pro Search?_ 문서](https://www.perplexity.ai/help-center/en/articles/10352903-what-is-pro-search)에서 가장 직접적으로 드러납니다. *"Every answer includes direct links to the original sources, allowing you to verify facts or explore further."* — **모든 답변에 원 출처로 가는 직접 링크가 포함된다**는 원칙. Standard Search와 Pro Search의 차이도 같은 문서에서 분명히 짚어요.

| 항목 | Standard Search | Pro Search |
|---|---|---|
| 검색 깊이 | _Surface-level_ | _Comprehensive_ |
| 출처 수 | 1~2 sources | dozens of sources |
| 인용 형태 | Basic citations | Extensive, direct source links |

마케터 시점에서 이건 중요한 차이예요. **Perplexity Pro는 _수십 개 출처_를 동시에 활용한다**는 뜻이고, 즉 _한 페이지만 잘 만든다고 인용이 보장되지 않아요_. _여러 권위 있는 출처에 같은 정보 패턴이 함께 등장_해야 Pro Search 답변에 들어갈 가능성이 올라가요.
...
**Gemini는 _Google Search를 직접 호출해서 그 결과를 인용하는 grounding 메커니즘_으로 답변해요.** Gemini API에서 인용은 _구조화된 메타데이터_로 반환된다는 점에서 다른 세 엔진과 가장 구조적으로 다른 차이를 보입니다.

[Google AI for Developers의 _Grounding with Google Search_ 문서](https://ai.google.dev/gemini-api/docs/google-search)가 핵심이에요. *"Grounding with Google Search connects the Gemini model to real-time web content and works with all available languages. This allows Gemini to provide more accurate answers and cite verifiable sources beyond its knowledge cutoff."* — _Gemini 모델을 실시간 웹 콘텐츠와 연결해서 검증 가능한 출처를 인용하게 만드는 기능_입니다.

작동 방식은 단순해요. `google_search` 도구를 켜면 *"the model handles the entire workflow of searching, processing, and citing information automatically"* — 모델이 검색·처리·인용을 _자동으로_ 수행합니다. 응답에는 세 메타데이터 필드가 포함돼요.

- `groundingMetadata` — grounding 작업 전체 메타데이터
- `groundingChunks` — 인용 대상 콘텐츠 청크 집합
- `groundingSupports` — _모델 출력 텍스트_ 와 _근거 소스_ 의 매핑

다른 세 엔진은 _문장 안에 인라인 마크_ 또는 _Sources 패널_로 인용을 _UI 차원_에서 보여주지만, **Gemini API는 _텍스트와 출처를 구조적으로 매핑한 데이터_를 반환**해요. 마케터 입장에서 이건 _Gemini를 통해 만든 콘텐츠·답변에서 어떤 청크가 어떤 출처에 매핑됐는지_ 추적·분석이 가장 정교하게 가능하다는 뜻이에요.

Gemini와 Google Search의 관계는 더 깊습니다. [Google Blog 2025-05-20 발표](https://blog.google/products-and-platforms/products/search/google-search-ai-mode-update/)에 따르면 *"Starting this week, we're bringing a custom version of Gemini 2.5, our most intelligent model, into Search for both AI Mode and AI Overviews in the U.S."* — **Gemini 2.5의 맞춤형 버전이 Google Search의 AI Mode와 AI Overviews에 직접 투입**돼요. 즉 _Gemini 앱_에서 받는 답변과 _Google SERP의 AI 답변 레이어_가 모델 계층에서 긴밀히 연결된다는 뜻이에요. 다만 사용자가 보는 _UI는 별개_ — Gemini 앱은 챗 UI, Google SERP은 AI Overview·AI Mode UI로 분리돼 있어요.

출처 성향은 다른 엔진과도 또 달라요. Profound 측정에서 **Gemini는 brand 비중이 73%로 가장 높았어요** — 광고주·사업자 사이트 같은 _브랜드 출처_를 가장 자주 인용하는 엔진입니다.

## Claude는 어떻게 답변하고 어떻게 인용하나요?

**Claude는 _문서·검색 결과 블록에 대한 정교한 citation pointers_와 _토큰 비용·구조 정책_을 가장 명시적으로 분리해 설명하는 엔진이에요.** Anthropic 공식 문서가 다른 세 엔진보다 _개발자 입장에서 가장 구체적_인 점이 특징입니다.

[Anthropic Citations 문서](https://docs.anthropic.com/en/docs/build-with-claude/citations)는 두 가지 정책을 짚어요. 첫째, _토큰 회계_. *"The cited_text field is provided for convenience and does not count towards output tokens. When passed back in subsequent conversation turns, cited_text is also not counted towards input tokens."* — 인용된 원문(`cited_text`)은 _출력 토큰에도 입력 토큰에도 포함되지 않아요_. 같은 인용을 반복 표시해도 토큰 비용 부담이 없다는 뜻이에요.

둘째, **Citations와 Structured Outputs는 함께 쓸 수 없어요**. *"Citations cannot be used together with Structured Outputs. If you enable citations on any user-provided document... and also include the output_config.format parameter (or the deprecated output_format parameter), the API will return a 400 error."* — 두 기능을 동시에 켜면 _400 에러_가 발생합니다. 이건 다른 세 엔진 공식 문서에서는 명시되지 않은 정책 레벨 제약이에요.

실시간 검색은 [Web search tool 문서](https://docs.anthropic.com/en/docs/build-with-claude/tool-use/web-search-tool)가 책임집니다. *"At the end of its turn, Claude provides a final response with cited sources."* — 검색 도구를 활성화하면 Claude가 _언제 검색할지_ 스스로 결정하고, _턴의 끝에서 인용 출처가 포함된 최종 응답_을 제공해요. 비용도 명확합니다 — **웹 검색은 1,000 검색당 $10**로 별도 과금됩니다. 최신 버전 `web_search_20260209`에서는 _dynamic filtering_이 가능한데, _code execution tool을 함께 활성화해야_ 작동하고 _Vertex AI에서는 미지원_입니다.

마케터 시점에서 Claude의 차별점은 두 가지예요. 첫째, _구조화 출력(JSON 등)과 인용을 동시에 못 쓰는_ 제약 때문에, _Claude를 자동화 파이프라인에 쓸 때 인용 부분은 별도 단계로 분리_해야 해요. 둘째, _토큰 회계가 가장 정교_하다는 건 _긴 문서를 반복 인용해도 비용 증가가 통제_된다는 뜻이고, 즉 _Claude는 깊이 있는 출처 활용에 가장 안정적_인 엔진이에요.

출처 성향은 _brand + media 혼합형_ — 광고주 사이트와 권위 매체를 균형 있게 인용해요. H2-5 비교에서 정확한 수치로 확인할게요.

## 네 엔진을 비교하면 인용 행태에 어떤 차이가 있나요?

**네 엔진은 _모두 출처 표시를 말하지만 어떤 종류의 출처를 우선 인용하느냐는 매우 다르게 갈려요._** Profound · Peec AI 두 산업 측정사의 분석 결과를 겹쳐 보면 _각 엔진의 출처 성향_이 한눈에 잡힙니다.

먼저 가장 자주 인용되는 4사 비교 데이터입니다. [Profound _Enhanced Citation Categories_ (2026-01-08)](https://www.tryprofound.com/blog/enhanced-citation-categories)는 출처를 **Brand · Media · Institution · Social** 네 카테고리로 분류한 결과를 발표했어요. 핵심 메시지는 한 문장으로 정리돼요 — *"Perplexity cites social at 19.4% compared to ChatGPT at 5.3%."* — **Perplexity는 Social형(Reddit · 커뮤니티) 출처 비중이 19.4%로, ChatGPT의 5.3%의 거의 4배**입니다.
...
각 엔진별 최다 인용 도메인도 [Profound _AI Platform Citation Patterns_ (2025-06-04)](https://www.tryprofound.com/blog/ai-platform-citation-patterns)에서 확인돼요. *"Wikipedia serves as ChatGPT's most cited source at 7.8% of total citations. Reddit emerges as the leading source for both Google AI Overviews (2.2%) and Perplexity (6.6%)."* — **ChatGPT 1위는 Wikipedia(7.8%), Perplexity 1위는 Reddit(6.6%), Google AI Overviews 1위도 Reddit(2.2%)**. 한 가지 추가로 짚을 점은 _Wikipedia 7.8%_가 _전체 인용의 비율_이고, _상위 10개 출처 중에서는 47.9%_로 더 압도적이라는 사실 — ChatGPT가 _Wikipedia에 얼마나 의존하는지_가 두 지표 모두에서 드러나요.

전체 풍경은 [Peec AI 3천만 소스 분석 (2026-03-31)](https://peec.ai/blog/top-domains-cited-by-ai-search-analysis-based-on-30m-sources)이 가장 넓게 잡습니다.
...
마지막으로 학술 단위의 이야기도 짚을게요. [ACL NAACL 2025 ReClaim 논문](https://aclanthology.org/2025.findings-naacl.55/)은 *"ReClaim provides sentence-level citations in long-form question-answering tasks."* — **문장 단위 인용**이 _passage-level · paragraph-level_ 인용보다 검증 가능성이 높고, _90% citation accuracy_를 달성했다고 보고했어요. 이건 GeoMoment 블로그 3편 [_청크 단위 처방_](/blog/chunk-prescription/)의 결론(_AI는 청크 단위로 인용한다_)과 정확히 같은 방향이에요. 즉 _4 엔진 모두_ 학술적으로도 _작은 단위의 검증 가능한 청크_를 인용 대상으로 삼는다는 합의가 있어요.

마케터에게 이 분석의 의미를 한 줄로 모으면 이렇습니다 — **같은 페이지를 만들어도 어떤 엔진에서 인용될지는 엔진마다 다르고, 그 차이는 _우리 페이지가 어떤 카테고리(Brand · Media · Institution · Social)에 속하는지_에 크게 좌우돼요.** 자사몰 페이지(Brand)에 집중한 회사는 Gemini에 강하고, 권위 매체 PR을 잘 쌓은 회사는 ChatGPT에 강하고, Reddit·커뮤니티에 노출 잘 되는 회사는 Perplexity에 강합니다.
...
ChatGPT 2,293만 명이라는 숫자는 _2위 Grok 153만 명_의 15배. _Perplexity 152만 + 뤼튼 135만 + Gemini 44만을 모두 합쳐도 480만 명_으로, 여전히 ChatGPT의 1/5에 못 미쳐요. 한국 마케터가 _AI 답변 엔진 가시성_을 측정·최적화한다면 **ChatGPT가 1순위, 그것도 압도적 1순위**입니다.

그런데 그게 끝이 아니에요. 앞서 H2-5에서 본 _4 엔진의 출처 카테고리 분포_가 결정적입니다. **ChatGPT에서 잘 인용되는 페이지의 특성(Brand 54% · Media 25% · Institution 15% · Social 5%)이 Perplexity의 특성(57/19/5/19)과 _정확히 같지 않아요_.** 즉 _ChatGPT에 잘 인용되는 페이지가 Perplexity에서도 같은 강도로 인용된다는 보장이 없어요_.

특히 _Social(Reddit · 커뮤니티 · 평판형)_ 카테고리의 비중 차이(ChatGPT 5% vs Perplexity 19%)는 _브랜드 운영 방식 자체_에 영향을 줘요. ChatGPT만 최적화한 마케팅 팀은 _Reddit · 디시인사이드 · 클리앙_ 같은 한국 UGC 플랫폼에서의 평판을 챙기지 않을 수 있고, 그러면 _Perplexity 답변에 들어갈 가능성이 줄어들어요_.

여기에 한 가지 더 — [Google Korea Blog _검색어로 돌아본 대한민국의 2025년_ (2025-12-04)](https://blog.google/intl/ko-kr/products/explore-get-answers/year-in-search-2025-kr/)이 짚은 한국 검색 행태 변화도 함께 봐야 해요. *"구글의 'AI 개요(AI Overview)'에 이어 도입된 'AI 모드(AI Mode)'는 이용자들이 가장 자연스럽다고 느끼는 언어로 복잡한 질문을 던지고, 꼬리에 꼬리를 무는 후속 질문을 통해 대화하듯 답을 찾아가는 새로운 경험을 제공합니다."* — 미국 기준으로는 AI Mode 도입 후 _탐색형 질문 +70%_, _자연어 실행형 +25%_ 증가가 관찰됐다고도 짚었어요. 즉 **Google SERP에서의 AI 답변(Gemini 기반)도 한국에서 실사용 단계**예요.

그래서 **GeoMoment가 권장하는 한국 마케터의 답변 엔진 측정 우선순위**는 이렇게 정리돼요.

1. **ChatGPT 우선 측정** — 한국 사용자 절대 다수 (2,293만 명)
2. **Perplexity · Grok 함께 측정** — 한국 2~3위, 그러나 ChatGPT와 _다른 출처 카테고리_ 선호
3. **Gemini는 Google SERP의 AI Overview · AI Mode를 통해 _간접 측정_** — 한국어 SERP에서 이미 노출 중
4. **Claude는 글로벌·영문 콘텐츠가 있다면 우선 추가** — 한국 단독 사용자 수는 작지만 _깊이 있는 분석_에 강해 영문 콘텐츠 보유 시 우선순위 상승
5. **4사 동시 측정으로 우선순위 검증** — _같은 질의를 4사에 던져 인용 도메인 차이_를 직접 확인해야 _ChatGPT 1위 가정_이 우리 산업·키워드에서도 유효한지 확정 가능

직접 적용 결과가 궁금하시면 [GeoMoment 무료 진단](/geo-diagnose.html)을 통해 _ChatGPT · Perplexity · Gemini · Claude 네 모델에서 우리 브랜드가 동시에 어떻게 인용되는지_ 측정하실 수 있어요. 청크 단위 점수·처방 카드도 함께 발송해 드립니다.

다음 글에서는 한 발 더 들어가서, **Schema.org & llms.txt 완전 가이드** — AI 크롤러가 우리 브랜드 엔티티와 청크 구조를 _기계적으로 읽을 수 있게_ 만드는 마크업 패턴을 정리할게요. 4 엔진 모두에서 _구조화 데이터_가 인용 적격성을 끌어올리는 공통 레버라는 점이 6편의 출발점이에요.

같은 질문, 다른 답변 — _4 엔진은 각자 다른 출처 철학을 가진 서로 다른 답변 엔진_이에요. 우리 브랜드가 _어느 엔진에서 어떤 출처로 인용되는지_가 마케팅의 새 단위입니다.
